...
namespace Business.FlyWheel3Business
{
    public class FlyWheel3Business
    {
        ...
        public FlyWheel3Business()
        {
            try
            {
                ...
                //电机ID
                string AxisID = System.Configuration.ConfigurationSettings.AppSettings["Motor3AxisIDString"];
                string[] axisPara = AxisID.Split('=');
                axis_IDnumber = axisPara[1].Trim();
                Defs.Motor3ID = axis_IDnumber;
            }
            catch (Exception ex)
            {
            ...
            string experiment_memeber, string experiment_product)
        {
            ...
            if (state == "ON")
            {
                ...
                {
                    WriteDataToDatabaseWave("3", "7", MotorCur[i].ToString(), dttime[i].ToString(), experiment_name, experiment_memeber, experiment_product);
                }
                #endregion
            ...
            else
            {
                Defs.ModifyState("3", '1');
                D2KDASK.D2K_DO_WritePort(0, D2KDASK.Channel_P1A, Convert.ToUInt32(Defs.StateVector, 2));
                Defs.Motor3RSuceess = false;
                //if (Defs.StateVector == "11111111")
                //{
                //    Defs.brun1 = false;
                //    Defs.brun2 = false;
            ...
            string RotorRevRea, string datetime, string experiment_name, string experiment_memeber, string experiment_product, string real_deta_moment)
        {
            ...
            string sql = "insert into dbo.Motor31(MotorID, Scheme, SouceVol, SouceCut, RotorVol, RotorCut, RotorPow, ConstantMoment, ChangeMoment, RotorRevIde, RotorRevRea, DateTime, backup1,backup2,backup3,backup4)";
    ...
    string RotorPow, string RotorRevRea, string MeanTorque, string datetime, string ideal_torque, string experiment_name, string experiment_memeber, string experiment_product)
        {
            string sInfo = string.Empty;
            string sql = "insert into dbo.Motor32(MotorID, Scheme, SouceVol, SouceCut, RotorVol, RotorCut, RotorPow, RotorRevRea, MeanTorque, datetime,backup1,backup2,backup3,backup4)";
        ...
        {
            string sInfo = string.Empty;
            string sql = "insert into dbo.Motor36(MotorID, Scheme, RotorCut,datetime,backup1,backup2,backup3)";
        ...
        {
            string sInfo = string.Empty;
            string sql = "insert into dbo.Motor33(MotorID, Scheme, SouceVol, SouceCut, RotorVol, RotorCut, RotorPow, RotorRevIde, RotorRevRea, datetime, backup1,backup2,backup3,backup4,backup5,backup6,backup7,backup8)";
    ...
    string RotorPow, string RotorRevRea, string MeanTorque, string datetime, string ideal_torque, string experiment_name, string experiment_memeber, string experiment_product, string Scheme4_torque)
        {
            string sInfo = string.Empty;
            string sql = "insert into dbo.Motor34(MotorID, Scheme, SouceVol, SouceCut, RotorVol, RotorCut, RotorPow, RotorRevRea, MeanTorque, datetime,backup1,backup2,backup3,backup4,backup5)";
        ...
        {
            string sInfo = string.Empty;
            string sql = "insert into dbo.Motor35(MotorID, Scheme, TimeConstant, datetime,backup1,backup2,backup3)";
